...
- **Page H1** (case study title, e.g. "Modernizing PROS' Revenue Management Platform"): `text-[44px] sm:text-[58px] text-[var(--color-cs-heading)] cs-lh-normal`, `weight 500` via a dedicated `.cs-page h1.cs-lh-normal { font-weight: 500 !important; }` rule in `index.css` (needed to win over the site-wide `.cs-page h1 { font-weight: var(--type-h1-weight) !important }` rule, which otherwise forces all case-study h1/h2/h3 to 400 regardless of inline styles). Note: `--font-display` (Quattrocento) is only loaded at weights 400/700, so 500 renders as the nearest loaded weight (400) until/unless Quattrocento:wght@500 is added to the Google Fonts import.
- **Chapter titles** (e.g. "3. Research"): number span — `font-landing-heading`, `28px`, `weight 300`, `color: var(--color-cs-heading)` (matches the heading text color); heading — `font-bold cs-editorial text-[var(--color-cs-heading)] cs-lh-normal`, `font-display`, `28px`, `weight 500`, not italicized (`cs-editorial` no longer applies italic styling). Every chapter has this number+heading row followed by the bordered divider bar (see "Layout patterns" below).
- **Subheadings** (`SubHeading` component): `24px`, `font-landing-heading`, `weight 500`, `lineHeight: normal`, color `var(--color-cs-heading)`. Same override mechanism as the H1 above — `.cs-page h3.cs-lh-normal { font-weight: 500 !important; }` in `index.css` wins over the blanket 400. This is the confirmed standard — don't revert either override.
- **Body text** (`BodyText` component): `17px`, `font-landing-body`, `color: var(--color-secondary)`, `lineHeight: normal`. Note: on case study pages, the site-wide `.cs-page p {...} !important` rule silently forces the *actual* rendered `<p>` values to `font-size: var(--type-body-size)` (17px — raised from 15px), `font-weight: var(--type-body-weight)` (350), `line-height: 1.2` (hardcoded, not `var(--type-body-line-height)` — that var stays 1.7 and is used site-wide elsewhere, so it was scoped here instead of changed globally), `color: #222225` regardless of inline/BodyText styles — this is the true computed body-text style and is now also the standard for `.case-study-card-description` on the landing page, for consistency across the whole site. `--type-body-size` itself is a shared variable, so raising it to 17px affects body copy everywhere it's referenced (nav links, footer, etc.), not just case study `<p>` tags.
