...
- **Wireframe-test carousel labels** (the caption identifying which option/state is showing inside a sliding `ImageCarousel`, e.g. "Destinations," "Entry Point"): use `cs-caption` below the image, not `cs-caption-label` above it — this is Fare Finder's explicit deviation from Expert.ai's `AccessibilityExplorer` (which puts a bold uppercase `cs-caption-label` above the image). Applies to plain single-visual carousels only, not option cards that pair a label with Pros/Cons (those keep the `cs-caption-label`-above-image pattern, e.g. RM's `MyMarketsExplorer`).
