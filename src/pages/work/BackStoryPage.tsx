...
function FeatureBlock({ index, label, body, userImpact, image, alt, first = false }: { index: number; label: string; body: string; userImpact?: string; image: string; alt: string; first?: boolean }) {
  return (
    <div className="bs-feature-grid" style={{ marginTop: first ? 0 : 108, display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 32, alignItems: 'center', justifyItems: 'center' }}>
      <img src={img(image)} alt={alt} style={{ maxWidth: '100%', width: 'auto', height: 'auto', display: 'block', borderRadius: 8 }} />
      <div>
